...
3. CAPTURE GAPS — for each of the last 7 days, list any day missing: a sleep entry (`log.sleep`), any caffeine entry, any meal entry, any routine ticks, any workout entry, any `log.focus_sessions` entry. These are the seeds for dashboard nudges (USER_FLOW_PLAN §7.8). Output as a small block: `Capture gaps (last 7d): sleep [d1, d3]; focus_sessions [d2..d7]; ...`.
3b. FOCUS OUTPUT (the Prime Objective — `user_profile.json` system rule + comprehensive_memory.md §6). From `log.focus_sessions[]` compute daily **Σ(quality × hours)** across the window; report the 7-day series + mean, and flag correlations with the upstream drivers (prior-night sleep duration, the post-lunch walk, morning focus-primer adherence). This is the headline metric the whole protocol optimizes — surface it first.
...
11. Append a `### HH:MM | System` entry with these sections in order: Focus output (Σ quality×hours 7d series + mean + driver correlations); Capture gaps; Open loops; Override confirmation; KB sync-walk (Method §K coverage line + any re-validation / drift flags); Proposed rule deltas (with evidence — which days, counts); Applied / Queued summary. Markdown only, no tables. Commit and push `user_profile.json` (if changed) and `journal/<date>.md` DIRECTLY to `main` (no branch, no PR).
```
