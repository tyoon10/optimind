...
3. Compute simple trends from the daily files: avg sleep quality and its delta vs the prior week, median bedtime/wake, caffeine count + how often after 14:00, routine-compliance %, workout count. Don't fabricate missing days — say "no data".
4. Append a `### HH:MM | System` report to journal/<date>.md with: Wins (with numbers), Drift (+ likely upstream cause), Open loops, and ONE concrete suggested focus for next week. Markdown, no tables. Then commit and push DIRECTLY to `main` (no branch, no PR).
```
