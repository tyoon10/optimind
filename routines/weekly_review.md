...
2a. INTEGRITY GATE — RUN BEFORE ANY COUNT. Execute `python3 scripts/audit_dual_write.py --since 14 --json`. Days classified `journal_only` / `structured_only` / `mixed` are WRITE DEFECTS: the behavior happened and the structured write was lost, so they must never lower a compliance percentage or appear as a capture gap. `needs_input` days are questions for the user, not misses. The current NYC date is excluded from every denominator. If any CLOSED day diverges, open the report with **STRUCTURALLY INCOMPLETE** and name the dates — a weekly trend computed over a broken record is not a finding.

3. STRUCTURED-LOG COUNTS (capture-completeness, not adherence) — count entries this week: sleep entries / total caffeine entries / meal entries by slot / routine ticks by item / workout entries. Count a field as captured when EITHER record holds it, and report the two sources separately where they differ. Output as one line: `Capture: sleep 4/7, caffeine 12, meals 18 (bk 5/lu 6/di 7), workouts 1, routine cold_shower 5/7 sunlight 3/7`, then `Integrity: matched X/Y closed days; lost writes [dates]; needs input [dates]`. This grounds nudge generation (USER_FLOW_PLAN §7.8).
